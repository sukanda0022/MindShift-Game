...
function startGameLoop() {
    ...
    gameInterval = setInterval(async () => {
        ...
        if (timeLeft > 0) {
            timeLeft--;
            if (!isBreakMode) {
                // ✨ แก้ไข Logic: ถ้าแค่จอดับ (isSleeping) พลังงานจะลดปกติ แต่สถานะใน Firebase ยังเป็น online
                if (isSleeping) {
                    periodEnergy -= 1.5;
                    if (periodEnergy <= 0) {
                        periodEnergy = 0;
...
// --- [⭐ ส่วนแก้ไขใหม่: แยกจอดับ กับ สลับแอป ⭐] ---

let isActuallyBlurred = false; 

window.addEventListener('blur', () => {
    if (!isBreakMode && gameInterval && !hasFailedPeriod) {
        isActuallyBlurred = true; // ยืนยันว่ามีการสลับแอปจริง
        isSleeping = true;
        tabSwitchCount++; 
        updateImage();
        // ฟ้องแอดมินทันทีว่า Away เพราะนี่คือการสลับแอป
...
document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
        // บันทึกเวลาไว้คำนวณพลังงานย้อนหลัง
        localStorage.setItem("lastExitTime", Date.now().toString());
        
        // 💡 ถ้าหน้าจอถูกซ่อนแต่ isActuallyBlurred เป็น false หมายถึง "จอดับเอง" หรือ "ปิดหน้าจอ"
        // เราจะไม่ส่ง Away เพื่อให้แอดมินเห็นว่านิสิตยังอยู่ในแอป
        if (!isActuallyBlurred) {
            console.log("💤 จอดับ: รักษาพยาบาลสถานะออนไลน์ไว้ใน Firebase");
            // ส่ง Online ซ้ำเพื่อความชัวร์ว่าแอดมินจะไม่เห็นเป็นส้ม
            updateOnlineStatus("online"); 
        }
    } else {
        // เมื่อเปิดจอขึ้นมาใหม่
        handleBackgroundTime();
        if (!isActuallyBlurred) {
            isSleeping = false;
            updateOnlineStatus("online");
